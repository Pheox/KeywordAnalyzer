...
class PageFeatures(object):
  ...
  def __repr__(self):
    ...
    s += 'Path %s' % self.path
    s += ': DA=%d PA=%d MR=%d PR=%d SR=%d' % (self.da, self.pa, self.mr,
                                              self.pr, self.sr)
    return s
...
class URLsAnalyzer(object):
  ...
  def __init__(self):
    ## Mozscape
    self._seomoz = Seomoz(URLsAnalyzer.seomoz_access_id,
        URLsAnalyzer.seomoz_secret_id)
...
class KeywordAnalyzer(object):
  ...
  def __init__(self, keyword):
    ...
    self._language = 'en'
    self._engine = GoogleSearch(self.keyword.encode('utf8'),
      lang=self._language, lang_restrict="lang_"+self._language)
    self._engine.results_per_page = 25

    ## Mozscape
    self._seomoz = Seomoz(KeywordAnalyzer.seomoz_access_id, KeywordAnalyzer.seomoz_secret_id)
  ...
  engine_position = property(fset=_set_engine_position,
                            fget=_get_engine_position)
  ...
  def _get_pages_urls(self):
    ...
    if searches_diff > 0:
      ...
      search_results = self._engine.get_results()[(self._engine_position % self._engine.results_per_page):]
    else:
      search_results = self._engine.get_results()[(self._engine_position % self._engine.results_per_page):]
