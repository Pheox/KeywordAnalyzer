...
from models import Keyword, KeywordPage
...
class TaskDAO(object):
    ...
    def set_task_state(task, state_str):
        state = TaskState.query.filter(
            TaskState.state_str == state_str).first()
        task.state = state
        db.session.commit()
    ...
    def add_task(title, date, state_str, priority=None, filter_id=None,
                 keyword_id=None, max_searches=None, language=None,
                 engine=None, hide_flag=None):
...
class KeywordPageDAO(object):
    ...
    def add_kw_page(task, page):
        ...
        if kw_page:
            ...
            kw_page = KeywordPage(
                keyword_id=task.keyword_id, page_id=page_db.id,
...
class FilterDAO(object):
    ...
    def get_filter(da_th, pa_th, mr_th, pr_th, sr_th):
        return Filter.query.filter(
            Filter.da_th == da_th, Filter.pa_th == pa_th,
            Filter.mr_th == mr_th, Filter.pr_th == pr_th,
            Filter.sr_th == sr_th).first()

    @staticmethod
    def add_filter(da_th, pa_th, mr_th, pr_th, sr_th):
        filter = Filter(
            da_th=da_th, pa_th=pa_th, mr_th=mr_th,
