...
from models import Task, Page, Domain, KeywordPage
...
class TaskDispatcher(threading.Thread):
  ...
  def _save_page(self, page):
      ...
      if page_db.count():
        ...
        page_db = Page(path=page.path, date=datetime.utcnow(), domain=domain,
                    pa=page.pa, mr=page.mr, pr=page.pr, sr=page.sr)
  ...
  def _run_kw_analyzer(self, task):
    ...
    searches_done = task.searches_done
